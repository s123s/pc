		...
		<div class="inner-wrapper">
			...
			<div class="center pane">
				...
				<div class="inner">
					...
					<div class="bottom">
						<a class="newPos"><span class="glyphicon glyphicon-asterisk"></span>&nbsp;Новый монитор</a><br>
						...
						<table id="mtab" class="table table-striped">
							<thead>
								<tr>
									...
									<th class="col-xs-3">Производитель</th>
									<th class="col-xs-3">Модель</th>
									<th class="col-xs-1">Разм. диагонали</th>
							...
							<tbody>
								...
								<c:forEach items="${monitors}" var="monitor" varStatus="st">

									<tr id="id${monitor.idMonitor}">
										<td class="myIndex" hidden="true">${st.getIndex()+1}</td>
										<td class="idMonitor">${monitor.idMonitor}</td>
										<td class="invNumber">${monitor.invNumber}</td>
										<td class="idProducer" hidden="true">${monitor.producer.idProducer}</td>
										<td class="producerName">${monitor.producer.shortname}</td>
										<td class="model">${monitor.model}</td>
										<td class="size">${monitor.size}</td>
...
<div id="editDialog" title="Редактирование монитора" class="dialogWindow" hidden=true>
	<form id="editDialogForm" action="" method="post">
	  <table>
	  		<tr hidden="true"><td><input type="text"  id="idTr"></td></tr>
				...
				<tr><td>Разм. диагонали</td><td><input type="text" id="size" name="size"></td></tr>
	  </table><br>
	</form>
	<button id="editDialogSave" class="ui-button ui-widget ui-corner-all">Сохранить</button>
</div>

<div id="newDialog" title="Новый монитор" class="dialogWindow" hidden=true>
	<table>
			<tr><td>Инв. номер</td><td><input type="text" id="invNumber" name="invNumber"></td></tr>
			<tr><td>Производитель</td><td>
				<select id="idProducer" style="width:100%">
...
<div id="deleteDialog" title="Удаление монитора" class="dialogWindow" hidden=true>
