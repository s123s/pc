...
public class MysqlProcessorDaoHibernate implements ProcessorDao { 
...
/*	@Override
	...
	public List<Processor> readAllFreeRows() {
		...
		for (Iterator<Processor> iter = rows.iterator(); iter.hasNext(); ) {
		    ...
		    if (processor.get.getProcessors().size() != 0)  {
		        iter.remove();
		    }
		}
		return rows;
	...
	public OperationStatus create(Processor o) {
		...
		try {
			...
			//session.persist(o);
			if (o.getComputer().getIdComputer() == null) o.setComputer(null);
			//o.setTypeProcessor(session.load(TypeProcessor.class, o.getTypeProcessor().getIdTypeHdd()));
			session.save(o);
