...
saveNewPosOnTrToHTML = function (saveAnswer, trElement) {
	...
	var $tr    = trElement;
	var idLocal = saveAnswer.retObject.idProcessor;

	 trElement.id = "id"+idLocal;
	 $('.idProcessor', trElement).text(idLocal);
	 $tr.class = "";
...
saveNewPosOnTrToDatabase = function (trElement){
	...
	var idTypeProc = $(".idTypeProc>select", trElement).val();

	var json = {  "idProcessor"	: "",
				computer:{"idComputer" : "", },
				typeProc:{"idTypeProc"		: idTypeProc, },
				};
...
saveEditedPosToDatabase = function (trElement){
	...
	var idProcessor = $(".idProcessor", trElement).text();
	var idComputer = $(".idComputer", trElement).text();
	var idTypeProc = $(".idTypeProc>select", trElement).val();

	var json = { "idProcessor"	: idProcessor,
				computer:{"idComputer" : idComputer, },
				typeProc:{"idTypeProc"		: idTypeProc, },
				};

	$.ajax({
		url: "processor/save_edited",
...
fillDeleteDialog = function (idTr) {
	var idProcessor = $("#mtab").find("#"+idTr).find(".idProcessor").text();

	$("#deleteDialog #idTr")[0].value = idTr;
	$("#deleteDialog #id")[0].value = idProcessor;
}
...
deleteFromDatabase = function (){
	...
	var json = { "idProcessor"	: idLocal };

	$.ajax({
		url: "processor/delete",
...
registerActions = function () {
	...
	$( ".idTypeProc select" ).on( "change", function( event, ui ) {
		saveEditedPosToDatabase(this.parentNode.parentNode);
	} );
