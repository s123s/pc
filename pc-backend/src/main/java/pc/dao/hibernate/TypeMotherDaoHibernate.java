...
public class TypeMotherDaoHibernate extends TypeMotherDao {
	...
	public List<TypeMother> readAll() {
		...
		return session
				.createQuery(
						"from TypeMother tab left outer join fetch tab.mothers left outer join fetch tab.producer order by tab.idTypeMother")
				.list();
	}
	...
	@Override
	@Transactional
	public OperationStatus create(TypeMother o) {
		Session session = getSessionFactory().getCurrentSession();
		try {
			System.out.println(o.getClass() + " updating");
			session.save(o);
